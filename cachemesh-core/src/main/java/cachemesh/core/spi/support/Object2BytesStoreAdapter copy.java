/*
 * Copyright © 2023 Yiting Qiang (qiangyt@wxcount.com)
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package cachemesh.core.spi.support;

import javax.annotation.Nonnull;
import javax.annotation.Nullable;
import static java.util.Objects.requireNonNull;

import cachemesh.common.misc.Holder;
import cachemesh.core.bean.Value;
import cachemesh.core.bean.ValueResult;
import cachemesh.core.bean.ValueStatus;
import cachemesh.core.config.CacheConfig;
import cachemesh.core.spi.BytesStore;
import cachemesh.core.spi.ObjectLoader;
import cachemesh.core.spi.ObjectMapper;
import cachemesh.core.spi.ObjectMapperAdapter;

public abstract class Object2BytesStoreAdapter implements BytesStore {

    private final CacheConfig config;

    public Object2BytesStoreAdapter(CacheConfig config) {
        this.config = config;
    }

    @Override
    @Nonnull 
    public CacheConfig getConfig() {
        return this.config;
    }


    @Nullable
    @SuppressWarnings({"rawtypes", "unchecked"})
    public ValueResult<byte[]> getSingle(@Nonnull String key, long version, @Nullable ObjectLoader loader) {
        requireNonNull(key);

        var cfg = getConfig();
        var serder = cfg.getSerder().getKind().instance;

        Value<Object> objV;
                
        if (loader == null) {
            objV = doGetSingle(key, null);
        } else {
            var valueClass = loader.getValueClass();
            if (valueClass == byte[].class) {
                var vholder = new Holder<Value<byte[]>>();

                objV = doGetSingle(key, ObjectLoader.of(valueClass, k -> {
                    Value<byte[]> v = (Value)loader.apply(k);
                    if (v == null) {
                        return null;
                    }
                    vholder.set(v);

                    Value<Object> newObjV;

                    var newBytes = v.getData();
                    if (newBytes == null) {
                        newObjV = new Value<>(null, v.getVersion());
                    } else {
                        Object newObj = serder.deserialize(newBytes, cfg.getValueClass());
                        newObjV = new Value<>(newObj, v.getVersion());
                    }

                    return newObjV;
                }));

                var v = vholder.get();
                if (v != null) {
                    return new ValueResult<>(ValueStatus.OK, v);
                }
            }
            
            objV = doGetSingle(key, loader);            
        }
        
        if (objV == null) {
            return null;
        }

        long storeVer = objV.getVersion();
        if (version > 0) {
            if (storeVer == version) {
                return (ValueResult<byte[]>) ValueResult.NO_CHANGE;
            }
        }

        Value<byte[]> v;

        var data = objV.getData();
        if (data == null) {
            v = new Value<>(null, storeVer);
        } else {
            var dataBytes = serder.serialize(data);
            v = new Value<>(dataBytes, storeVer);
        }

        return new ValueResult<>(ValueStatus.OK, v);        
    }

    @Nullable 
    protected abstract Value<Object> doGetSingle(@Nonnull String key, @Nullable ObjectLoader loader);

    @Override
    public void putSingle(@Nonnull String key, @Nullable byte[] value) {
        requireNonNull(key);

        var cfg = getConfig();
        var serder = cfg.getSerder().getKind().instance;

        doPutSingle(key, ObjectMapperAdapter.of(cfg.getValueClass(), (k, oldValue) -> {
            long ver = (oldValue == null) ? 1 : oldValue.getVersion();

            if (value == null) {
                return new Value<>(null, ver);
            }

            var data = serder.deserialize(value, cfg.getValueClass());
            return new Value<>(data, ver);
        }));
    }

    protected abstract void doPutSingle(@Nonnull String key, @Nonnull ObjectMapper mapper);

    @Override
    public void removeSingle(@Nonnull String key) {
        requireNonNull(key);
        doRemoveSingle(key);
    }

    protected abstract void doRemoveSingle(@Nonnull String key);

}
